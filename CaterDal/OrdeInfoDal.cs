...
namespace CaterDal
{
    public partial class OrdeInfoDal
    {
        ...
        public int DianCai(int orderid, int dishId)
        {
            ...
            if (count > 0)
            {
                //这个订单已经点过这个菜，如果点过一次，在点，就让订单数量加1
        ...
        {
            //orderDetailInfo数据表，count数量列，oid列Id编号
        ...
        public List<OrderDetailInfo> GetDetaiList(int orderId)
        {
        ...
        //设置下单按钮
        public int SetOrderMomery(int orderid, decimal money)
        {
        ...
        {
            //创建数据库的链接对象
            using (SQLiteConnection conn = new SQLiteConnection(System.Configuration.ConfigurationManager.ConnectionStrings["itcaseCater"].ConnectionString))
            {
                int result = 0;
                //由数据库链接对象创建事务
                conn.Open();
